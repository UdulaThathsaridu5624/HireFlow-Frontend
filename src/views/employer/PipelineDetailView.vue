<template>
  <AppShell>
    <div v-if="pipelineStore.loading" class="flex items-center justify-center py-20 text-muted-foreground/60">
      <Loader2 class="h-6 w-6 animate-spin mr-2" /> Loading...
    </div>
    <div v-else-if="!pipeline" class="py-20 text-center text-muted-foreground/60">Pipeline not found.</div>
    <div v-else class="flex flex-col gap-6">
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-xl font-bold text-foreground">{{ pipeline.candidateName ?? 'Candidate' }}</h1>
          <p class="text-sm text-muted-foreground">{{ pipeline.jobTitle ?? 'Position' }}</p>
        </div>
        <Badge :variant="pipeline.currentStage === HiringStage.REJECTED ? 'destructive' : 'outline'" class="ml-auto">
      ...
      <Card>
        <CardHeader>
          <CardTitle class="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Hiring Progress</CardTitle>
        </CardHeader>
        <CardContent>
      ...
      <!-- Advance Stage -->
      <Card v-if="pipeline.currentStage !== HiringStage.HIRED && pipeline.currentStage !== HiringStage.REJECTED">
        <CardHeader>
          <CardTitle class="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Move to Stage</CardTitle>
        </CardHeader>
      ...
      <div>
        ...
        <div v-if="!pipelineInterviews.length" class="text-sm text-muted-foreground/60">No interviews scheduled yet.</div>
        <div class="space-y-4">
          <InterviewCard
            v-for="interview in pipelineInterviews"
            :key="interview.id"
            :interview="interview"
          />
        </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle class="text-sm font-semibold text-muted-foreground uppercase tracking-wide">Stage History</CardTitle>
        </CardHeader>
        <CardContent class="space-y-2">
          ...
          >
            <span class="text-muted-foreground/60 text-xs w-32 shrink-0">{{ formatDate(entry.changedAt) }}</span>
...
import { HiringStage, STAGE_LABELS, HIRING_STAGE_ORDER } from '@/constants'
...
const pipelineInterviews = computed(() =>
  interviewStore.interviews.filter(
    (i) => pipeline.value && i.candidateId === pipeline.value.candidateId && i.jobId === pipeline.value.jobId,
  ),
)
