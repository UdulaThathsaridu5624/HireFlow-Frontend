      ...
      <section class="relative overflow-hidden bg-primary-container rounded-xl p-8 md:p-10 text-white">
        <div class="relative z-10 max-w-xl">
          ...
          <p class="text-white/70 text-sm md:text-base leading-relaxed">
            Ready for your next career move? Track your applications, prepare for interviews, and land the role you deserve.
          </p>
        ...
        <div class="absolute left-[-5%] bottom-[-20%] w-48 h-48 bg-white/5 rounded-full blur-[60px]"></div>
      </section>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12">
        ...
        <div class="lg:col-span-2 space-y-6">
          <div class="flex items-center justify-between">
            <h2 class="font-headline text-xl font-extrabold text-foreground">Recent Applications</h2>
            <button
          ...
          <div v-else class="space-y-3">
            <div
              ...
              @click="router.push(ROUTES.CANDIDATE_PIPELINE)"
            >
              <div class="w-12 h-12 bg-muted rounded-lg flex-shrink-0 flex items-center justify-center">
                <Briefcase class="h-6 w-6 text-primary" />
              </div>
              <div class="flex-1 min-w-0">
                <h4 class="font-headline text-sm font-bold text-foreground group-hover:text-primary transition-colors truncate">
                  {{ p.jobTitle ?? 'Position' }}
                </h4>
                <p class="text-muted-foreground text-xs mt-0.5">{{ p.companyName ?? 'Company' }}</p>
              </div>
              <div class="flex items-center">
                <Badge :variant="p.currentStage === HiringStage.REJECTED ? 'destructive' : 'secondary'" class="text-xs">
        ...
        <div class="space-y-6">
          ...
          <div class="bg-card p-6 rounded-xl editorial-shadow">
            <h3 class="font-headline text-base font-extrabold text-foreground mb-4">Next Interview</h3>
            ...
            <div v-else class="bg-surface-low rounded-xl p-4 border-l-4 border-tertiary-fixed-dim space-y-3">
              ...
              <div class="flex items-center gap-3 text-sm text-muted-foreground">
                ...
                <span>{{ formatInterviewTime(nextInterview.scheduledAt) }} ({{ nextInterview.durationMinutes ?? 45 }} min)</span>
              </div>
              <div class="flex items-center gap-3 text-sm text-muted-foreground">
                <Video class="h-4 w-4 text-primary shrink-0" />
                <span>{{ nextInterview.format === 'ONLINE' ? 'Online Meeting' : 'In Person' }}</span>
              </div>
              <a
          ...
          <div class="bg-card p-6 rounded-xl editorial-shadow">
            <div class="flex justify-between items-center mb-4">
              <h3 class="font-headline text-base font-extrabold text-foreground">Profile Strength</h3>
              <span class="text-primary font-black text-sm">{{ profileStrength }}%</span>
            </div>
            <div class="w-full h-2 bg-surface-high rounded-full overflow-hidden mb-5">
              <div
            ...
            <ul class="space-y-3">
              ...
              >
                <CheckCircle v-if="item.done" class="h-4 w-4 text-tertiary-fixed-dim shrink-0 mt-0.5" />
...
const offerCount = computed(() =>
  pipelineStore.pipelines.filter(p => p.currentStage === HiringStage.OFFER || p.currentStage === HiringStage.HIRED).length
)
...
const profileChecklist = computed(() => {
  ...
  return [
    ...
    },
    {
      ...
      hint: profile?.workExperience?.length ? `${profile.workExperience.length} roles documented` : 'Add work history',
